...
@Command(scope = "onos", name = "roadm-xc",
        description = "Adds/Drops cross-connection to/from the ROADM")
public class RoadmCrossConnectCommand extends AbstractShellCommand {
    ...
    @Argument(index = 7, name = "channelSpacing",
            description = "Channel spacing in [GHz]. Could be 0, 6.25, 12.5, 25, 50, 100 GHz",
            required = false, multiValued = false)
    ...
    protected OchSignal createOchSignal(String frequency, String sw, String grid, String spacing) {
        ...
        if (grid.toUpperCase().equals("FLEX")) {
            gridType = GridType.FLEX;
        }
        if (grid.toUpperCase().equals("UNKNOWN")) {
            gridType = GridType.FLEX;
        }
        if (grid.toUpperCase().equals("CWDM")) {
            gridType = GridType.CWDM;
        }
        if ((grid.toUpperCase().equals("DWDM")) || (grid == null)) {
            gridType = GridType.DWDM;
        }

        ChannelSpacing channelSpacing = null;
        if (spacing.equals("0")) {
            channelSpacing = ChannelSpacing.CHL_0GHZ;
        }
        if (spacing.equals("6.25")) {
            channelSpacing = ChannelSpacing.CHL_6P25GHZ;
        }
        if (spacing.equals("12.5")) {
            channelSpacing = ChannelSpacing.CHL_12P5GHZ;
        }
        if (spacing.equals("25")) {
            channelSpacing = ChannelSpacing.CHL_25GHZ;
        }
        if ((spacing.equals("50")) || (channelSpacing == null)) {
            channelSpacing = ChannelSpacing.CHL_50GHZ;
        }
        if (spacing.equals("100")) {
