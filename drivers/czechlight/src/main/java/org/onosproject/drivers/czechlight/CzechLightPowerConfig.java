...
public class CzechLightPowerConfig<T> extends AbstractHandlerBehaviour
        ...
        switch (deviceType()) {
            ...
            case ADD_DROP_FLEX:
                if (port.toLong() == CzechLightDiscovery.PORT_COMMON) {
                    return Optional.ofNullable(fetchLeafDouble(CzechLightDiscovery.NS_CZECHLIGHT_ROADM_DEVICE, "aggregate-power/common-out"));
                } else {
                    return Optional.ofNullable(fetchLeafSum(CzechLightDiscovery.NS_CZECHLIGHT_ROADM_DEVICE,
                            "media-channels[drop/port = '" + CzechLightDiscovery.leafPortName(deviceType(), port.toLong()) + "']/power/leaf-out"));
                }
            case COHERENT_ADD_DROP:
                ...
                if (port.toLong() == CzechLightDiscovery.PORT_COMMON) {
                    return Optional.ofNullable(fetchLeafDouble(CzechLightDiscovery.NS_CZECHLIGHT_COHERENT_A_D, "aggregate-power/express-out"));
                } else {
                    return Optional.ofNullable(fetchLeafDouble(CzechLightDiscovery.NS_CZECHLIGHT_COHERENT_A_D, "aggregate-power/drop"));
                }
            case INLINE_AMP:
                return Optional.ofNullable(fetchLeafDouble(CzechLightDiscovery.NS_CZECHLIGHT_INLINE_AMP, inlineAmpStageNameFor(port) + "/optical-power/output"));
        }
        return Optional.empty();
    }
    ...
    public Optional<Double> currentInputPower(PortNumber port, T component)  {
        ...
        switch (deviceType()) {
            ...
            case ADD_DROP_FLEX:
                if (port.toLong() == CzechLightDiscovery.PORT_COMMON) {
                    return Optional.ofNullable(fetchLeafDouble(CzechLightDiscovery.NS_CZECHLIGHT_ROADM_DEVICE, "aggregate-power/common-in"));
                } else {
                    return Optional.ofNullable(fetchLeafSum(CzechLightDiscovery.NS_CZECHLIGHT_ROADM_DEVICE,
                            "media-channels[add/port = '" + CzechLightDiscovery.leafPortName(deviceType(), port.toLong()) + "']/power/leaf-in"));
                }
            case COHERENT_ADD_DROP:
                if (component instanceof OchSignal) {
                    log.debug("Coherent Add/Drop: cannot query per-MC channel power");
                ...
                if (port.toLong() == CzechLightDiscovery.PORT_COMMON) {
                    return Optional.ofNullable(fetchLeafDouble(CzechLightDiscovery.NS_CZECHLIGHT_COHERENT_A_D, "aggregate-power/express-in"));
                } else {
                    return Optional.ofNullable(fetchLeafDouble(CzechLightDiscovery.NS_CZECHLIGHT_COHERENT_A_D, "client-ports[port='" + Long.toString(port.toLong()) + "']/input-power"));
                }
            case INLINE_AMP:
                return Optional.ofNullable(fetchLeafDouble(CzechLightDiscovery.NS_CZECHLIGHT_INLINE_AMP, inlineAmpStageNameFor(port) + "/optical-power/input"));
        }
        return Optional.empty();
    }
    ...
    public Optional<Range<Double>> getTargetPowerRange(PortNumber portNumber, T component) {
        //log.debug("CzechLight getTargetPowerRange {}", portNumber);
        return Optional.empty();
    }

    @Override
    public Optional<Range<Double>> getInputPowerRange(PortNumber portNumber, T component) {
        //log.debug("CzechLight getInputPowerRange {}", portNumber);
        return Optional.empty();
    }
