/*
 * Copyright 2019 Jan Kundrát, CESNET, <jan.kundrat@cesnet.cz>
 *
...
public class CzechLightDiscovery
    ...
    private static final String MOD_COEHRENT_A_D = "czechlight-coherent-add-drop";
    ...
    public static final String NS_CZECHLIGHT_COHERENT_A_D = NS_CZECHLIGHT_PREFIX + MOD_COEHRENT_A_D;
    ...
    public static final String channelDefinitionsFilter =
            "<channel-plan xmlns=\"http://czechlight.cesnet.cz/yang/czechlight-roadm-device\">" +
                    "<channel><lower-frequency/><upper-frequency/></channel>" +
                    "</channel-plan>";
    private static final String unidirCfgSubstr = "<port/><attenuation/><power/>";
    public static final String xmlMCOpen = "<media-channels xmlns=\"http://czechlight.cesnet.cz/yang/czechlight-roadm-device\">";
    public static final String xmlMCClose = "</media-channels>";
    public static final String mcRoutingFilter =
            xmlMCOpen +
                    "<add>" + unidirCfgSubstr + "</add>" +
                    "<drop>" + unidirCfgSubstr + "</drop>" +
                    xmlMCClose;
    ...
    public DeviceDescription discoverDeviceDetails() {
        ...
        try {
            ...
            var data = doGetXPath(getNetconfSession(), "yanglib", "urn:ietf:params:xml:ns:yang:ietf-yang-library",
                    "/yanglib:modules-state/module[(name='" + MOD_ROADM_DEVICE + "') or (name='"
                            + MOD_COEHRENT_A_D + "') or (name='" + MOD_INLINE_AMP +"')]");
            if (!data.containsKey(YANGLIB_KEY_REVISION)) {
                log.error("Not talking to a supported CzechLight device, is that a teapot?");
                return new DefaultDeviceDescription(handler().data().deviceId().uri(), Device.Type.OTHER,
            ...
            if (data.getString(YANGLIB_KEY_MODULE_NAME).equals(MOD_ROADM_DEVICE)) {
                if (!revision.equals("2019-09-30")) {
                    log.error("Revision mismatch for YANG module {}: got {}", MOD_ROADM_DEVICE, revision);
                    return new DefaultDeviceDescription(handler().data().deviceId().uri(), Device.Type.OTHER,
                            null, null, null, null, null, annotations.build());
                }
                final var features = data.getStringArray("data.modules-state.module.feature");
                isLineDegree = Arrays.stream(features)
                        .anyMatch(s -> s.equals("hw-line-9"));
                isAddDrop = Arrays.stream(features)
                        .anyMatch(s -> s.equals("hw-add-drop-20"));
                if (!isLineDegree && !isAddDrop) {
                    log.error("Device type not recognized, but czechlight-roadm-device YANG model is present. Reported YANG features: {}", String.join(", ", features));
                    return null;
                }
            } else if (data.getString(YANGLIB_KEY_MODULE_NAME).equals(MOD_COEHRENT_A_D)) {
                if (!revision.equals("2019-09-30")) {
                    log.error("Revision mismatch for YANG module {}: got {}", MOD_COEHRENT_A_D, revision);
                    return null;
                }
    ...
    public List<PortDescription> discoverPortDetails() {
        ...
        if (deviceType == DeviceType.INLINE_AMP) {
            DefaultAnnotations.Builder annotations = DefaultAnnotations.builder();
            annotations.set(AnnotationKeys.PORT_NAME, "Line West");
            ...
            annotations.set(AnnotationKeys.PORT_NAME, "Line East");
            annotations.set(OdtnDeviceDescriptionDiscovery.PORT_TYPE, OdtnDeviceDescriptionDiscovery.OdtnPortType.LINE.toString());
...

        {
            ...
            switch (deviceType) {
                case LINE_DEGREE:
                    annotations.set(AnnotationKeys.PORT_NAME, "Line");
                    annotations.set(OdtnDeviceDescriptionDiscovery.PORT_TYPE, OdtnDeviceDescriptionDiscovery.OdtnPortType.LINE.toString());
                    break;
                case ADD_DROP_FLEX:
                case COHERENT_ADD_DROP:
                    annotations.set(AnnotationKeys.PORT_NAME, "Express");
                    break;
            }
    ...
    public static HierarchicalConfiguration doGetXPath(final NetconfSession session, final String prefix, final String namespace, final String xpathFilter) throws NetconfException {
    ...
    public static HierarchicalConfiguration doGetSubtree(final NetconfSession session, final String subtreeXml) throws NetconfException {
    ...
    /** Massage an XPath fragment (without the "/module:" prefix) into a key suitable for XmlConfigParser.getString(...) access
     *
    ...
    public static String xpathToXmlKey(final String xpath) {
        return "data." // prefix added by RPC handling
                + xpath.replace('/', '.') // turn XPath level delimiters into XmlConfigParser's key format
                .replaceAll("\\[[^]]*\\]", ""); // filter out XPath list keys/selectors, they are not visible in XmlConfigParser
    }
